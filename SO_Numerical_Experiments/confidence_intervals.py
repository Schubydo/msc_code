...
def compare_acquisition_functions(ucb, ucb_ci, ei, ei_ci, pi, pi_ci, filename='acquisition_function_comparisons.csv'):
    ...
    # Create comparisons and results
    comparisons = [
        ('UCB', final_ucb, final_ucb_ci, 'EI', final_ei, final_ei_ci),
        ('UCB', final_ucb, final_ucb_ci, 'PI', final_pi, final_pi_ci),
        ('EI', final_ei, final_ei_ci, 'UCB', final_ucb, final_ucb_ci),
        ('EI', final_ei, final_ei_ci, 'PI', final_pi, final_pi_ci),
        ('PI', final_pi, final_pi_ci, 'UCB', final_ucb, final_ucb_ci),
        ('PI', final_pi, final_pi_ci, 'EI', final_ei, final_ei_ci),
    ]

    results = []
    for acq1, final1, ci1, acq2, final2, ci2 in comparisons:
        comparison_label = f'{acq1} > {acq2}'
        is_greater = 1 if final1 > final2 else 0
        superior = 1 if final1 - ci1 > final2 + ci2 else 0
        results.append({
